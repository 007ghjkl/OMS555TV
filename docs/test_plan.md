...
> 状态：开发中基线 0.2（已同步 TASK-005 VCP/UART 证据）
...
- TASK-005 已证明 USART2 VCP 下的 Modbus RTU 帧和寄存器行为，但不能替代 Host Phase 3 联调或 RS485 电气层测试。
- 标准 Modbus Client 未必能发送 CRC 错误帧，可能需要后续 Raw Frame 接口。
- 8/24 小时稳定性测试只能在具备持续硬件环境后执行。

## 8. 结果真实性

未实际运行的测试不得标记通过。TASK-003 与 TASK-005 已执行项目只能按各自记录标记通过；Host Phase 3、真实 RS485、系统恢复和长时稳定性在实际执行前必须继续标记“未验证”，不得从 Firmware VCP 结果外推。
