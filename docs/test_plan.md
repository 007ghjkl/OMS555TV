...
> 状态：开发中基线 0.7（已同步 TASK-012 验证结果）
...
- TASK-011 的应用状态与监控核心、TASK-012 的实时 UI 均已通过自动化测试；TASK-012 另完成 30 分钟真实 RS485、设备复位、陈旧状态、UI 响应和显式重连验收。TASK-013 的参数配置、通信调试和会话日志尚未实施或验收。
- 标准 Modbus Client 未必能发送 CRC 错误帧，可能需要后续 Raw Frame 接口。
- 8/24 小时稳定性测试只能在具备持续硬件环境后执行。

## 8. 结果真实性

未实际运行的测试不得标记通过。TASK-003、TASK-005、TASK-008、TASK-009、TASK-010、TASK-011 和 TASK-012 只能按各自记录标记通过；TASK-012 不代表 TASK-013 已验证。当前短距离 RS485 的 30 分钟结果不得外推为 8/24 小时稳定性、工业长线、隔离或 EMC 验证。
