...
> 状态：Phase 5 基础套件已实测；Phase 6 Schema v2、覆盖模型与复合/稳定性执行核心已完成，正式分类套件尚未实施。
...
当前目录列出 24 个候选用例，不代表全部已实现或已通过。TASK-016 的 `testcases/functional/phase5-smoke.json` 已在 COM6/真实 RS485 执行 8 条：A/B/C/环境温度与光敏分别做范围断言，Firmware 版本做序列断言，非法地址期望 0x02，A 相阈值执行写入、独立回读和原值恢复。表中与这些行为直接对应的 6 个候选项已标记 Phase 5 实机通过；B/C/环境温度作为 Phase 5 套件附加用例记录。`TC-F002` 的四温度多寄存器动态范围断言仍未落地，因为 v1 Schema 对多寄存器只支持精确序列，不能用本次单寄存器范围测试冒充。

Phase 6 已规划为 TASK-017～TASK-020。TASK-017 已完成 Schema v2、覆盖矩阵、严格 Loader、规范化模型和纯数据断言；TASK-018 仍需实现复合/超时/稳定性执行，TASK-019 才形成不少于 20 条有效正式套件，TASK-020 再完成真实 RS485 全量验收。Phase 6 的自动恢复仅覆盖无需人工操作的错误后恢复；RS485 物理拔插、STM32 Reset 与传感器操作仍属于 Phase 7。表中未通过项是候选用例，不能因输入模型完成而标记通过。
