...
> 状态：Phase 5/6 用例规划基线；具体数据范围以确认后的寄存器表和对应 Technical Spec 为准。
...
当前目录列出 24 个候选用例，不代表已实现或已通过。TASK-014～TASK-016 仅负责建立 Phase 5 输入契约、TestEngine 和至少 5 条真实 RS485 最小套件；其余候选用例与完整 20 条有效套件仍属于后续 Phase 6。用例 JSON 应在对应执行器和数据范围确认后逐批落地。
