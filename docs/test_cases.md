...
> 状态：Phase 5 基础套件已实测；Phase 6 完整分类套件任务已规划、尚未实施。
...
Phase 6 已规划为 TASK-017～TASK-020：先新增 Schema v2 与覆盖矩阵，再实现复合/超时/稳定性执行，随后形成不少于 20 条有效套件并完成真实 RS485 全量验收。Phase 6 的自动恢复仅覆盖无需人工操作的错误后恢复；RS485 物理拔插、STM32 Reset 与传感器操作仍属于 Phase 7。上述任务尚未派发或实施，表中未通过项保持原状态。
